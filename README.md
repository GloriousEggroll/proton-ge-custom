...
| `fsr2`               | `PROTON_FSR2_UPGRADE`          | Redirects games that ship their own FSR 2 upscaler (`ffx_fsr2_api_x64.dll` loaded dynamically from the game's own install directory) to a wine builtin that translates FSR2 calls into FSR4, reusing the `amdxcffx64.dll` driver downloaded for `PROTON_FSR4_UPGRADE`. The driver version can be supplied as the value instead of `1`, like so `PROTON_FSR2_UPGRADE="4.1.1"`; an explicit `PROTON_FSR4_UPGRADE` version takes precedence. Requires FSR4-capable hardware (RDNA4, or RDNA3 via `PROTON_FSR4_RDNA3_UPGRADE`); has no effect on games whose FSR2 integration is statically linked into the executable. Experimental.                                                                                                                                                                                                                                                                               |
