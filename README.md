...
|                       | <tt>PROTON_CMDLINE</tt> | Replaces the %command% and arguments with a new line. If the executable path contains spaces, don't forget to add a backslash before the space. |
|                       | <tt>PROTON_CMDLINE_PREPEND</tt> | Allow to use wrappers like `mangohud`. Requires disabling runtime. |
