...
| `fsr2`               | `PROTON_FSR2_UPGRADE`          | Redirects games that ship their own FSR 2 upscaler (`ffx_fsr2_api_x64.dll` loaded dynamically from the game's own install directory) to a wine builtin that translates FSR2 calls into FSR4, reusing the `amdxcffx64.dll` driver downloaded for `PROTON_FSR4_UPGRADE`. The driver version can be supplied as the value instead of `1`, like so `PROTON_FSR2_UPGRADE="4.1.1"`; an explicit `PROTON_FSR4_UPGRADE` version takes precedence. Only the DX12 renderer is translated: `amdxcffx64.dll` is the DX12 FidelityFX provider and has no Vulkan equivalent. Anything the builtin cannot translate — a Vulkan backend, hardware without FP8/WMMA — is forwarded to the FSR2 implementation the game shipped, so a game that lets you switch renderers can be left with the option enabled. The FidelityFX SDK DX12 modules that `PROTON_FFX4_UPGRADE` installs are downloaded automatically and pinned to the same version, because `amdxcffx64` refuses to hand out an FSR4 provider unless they are already loaded in the process. Requires FSR4-capable hardware (RDNA4, or RDNA3 via `PROTON_FSR4_RDNA3_UPGRADE`); has no effect on games whose FSR2 integration is statically linked into the executable. Experimental.                                                                                                                                                                                                                                                                               |
